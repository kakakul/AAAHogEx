...
/*		local result = AIRoad.CanBuildConnectedRoadPartsHere(HgTile.XY(221,203).tile, HgTile.XY(222,203).tile, HgTile.XY(221,204).tile);
	...
	function GetPassengerServedStationIds(paxCargo) {
		local served = {};
	...
	function GetPassengerServedTowns(paxCargo) {
		local served = {};
		foreach(route in Route.GetAllRoutes()) {
			if(!route.HasCargo(paxCargo)) continue;
	...
	function FindBestProfitableRouteForTown(unservedTown, connected, paxCargo) {
	...
	function FindNearestConnectedTown(unservedTown, connected) {
		local townLoc = AITown.GetLocation(unservedTown);
		local bestTown = -1;
	...
	function ConnectUnservedTowns() {
		...
		local served = GetPassengerServedTowns(paxCargo);
		...
		foreach(unservedTown in unserved) {
			...
			local candidate = FindBestProfitableRouteForTown(unservedTown, connected, paxCargo);
			...
			if(candidate != null) {
				...
				DoInterval();
				local builder = CreateBuilder(candidate, pendingPlans, dirtyPlaces, limitDate);
				if(builder != null) {
				...
				DoInterval();
				local builder = CreateBuilder(busCandidate, pendingPlans, dirtyPlaces, limitDate);
				if(builder != null) {
	...
	/*			candidate.score += AIBase.RandRange(10); // ほかのHogexとの競合を防ぐ
		...
		while(routeCandidates.Count() >= 1){
			...
			// Global connectivity: skip pax/mail routes where neither endpoint has an existing
			// station in the network (would create an isolated island of new stations).
			if(servedStationIds != null && servedStationIds.len() > 0
					&& (!("typeName" in t) || t.typeName == "route")
					&& (t.src instanceof TownCargo) && (t.dest instanceof TownCargo)
					&& (AICargo.HasCargoClass(t.cargo, AICargo.CC_PASSENGERS) || AICargo.HasCargoClass(t.cargo, AICargo.CC_MAIL))) {
				local srcConnected = false;
				foreach(sg, _ in t.src.GetStationGroups()) {
					if(servedStationIds.rawin(sg.id)) { srcConnected = true; break; }
				}
				if(!srcConnected) {
					local destConnected = false;
					foreach(sg, _ in t.dest.GetStationGroups()) {
						if(servedStationIds.rawin(sg.id)) { destConnected = true; break; }
					}
					if(!destConnected) {
						HgLog.Info("GlobalConnect: skip isolated route "+t.explain);
						continue;
					}
				}
			}
			...
			if(newRoutes.len() >= 1) {
				...
				foreach(newRoute in newRoutes) {
					...
					if(finalDestRoute != null && finalDestRoute.IsBiDirectional()) lastTransferCandidates.rawdelete(finalDestRoute.id);
				...
				// Update served station IDs so subsequent candidates in this step can connect to newly added stations
				if(servedStationIds != null) {
					foreach(newRoute in newRoutes) {
						if(newRoute.srcHgStation != null && newRoute.srcHgStation.stationGroup != null) {
							servedStationIds.rawset(newRoute.srcHgStation.stationGroup.id, true);
						}
						if(newRoute.destHgStation != null && newRoute.destHgStation.stationGroup != null) {
							servedStationIds.rawset(newRoute.destHgStation.stationGroup.id, true);
						}
					}
				}
	...
	function CreateExchangeAirsBuilder(t, dirtyPlaces) {
		...
		foreach(stationId in t.stations) {
			...
			if(dirtyPlaces.rawin(station.place.GetFacilityId()+":"+t.cargo)) {
				HgLog.Info("dirtyPlaces "+station.place+" "+t.explain);
				return null;
			}
		}
	...
	function CreateRouteCandidates(cargo, orgPlace, placeInfo, additionalProduction=0, maxResult=8, options={}) {
		...
		foreach(candidate in candidates) {
			...
			if(candidate.vehicleType == AIVehicle.VT_RAIL) {
				if(candidate.estimate.isSingle && !roiBase) {
