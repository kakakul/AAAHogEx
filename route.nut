...
class Route {
	...
	routeTraceId = null;
	reduceStrikes = null;
	vehicleBuyBlockedUntil = null;
	vehicleBuyBlockReason = null;
	...
	function Load(t) {
		...
		reduceStrikes = t.rawin("reduceStrikes") ? t.reduceStrikes : {};
		vehicleBuyBlockedUntil = t.rawin("vehicleBuyBlockedUntil") ? t.vehicleBuyBlockedUntil : 0;
		vehicleBuyBlockReason = t.rawin("vehicleBuyBlockReason") ? t.vehicleBuyBlockReason : null;
		allRoutes.rawset(id,this);
	}

	function SaveTo(t) {
		t.id <- id;
		t.routeTraceId <- routeTraceId;
		t.reduceStrikes <- reduceStrikes;
		t.vehicleBuyBlockedUntil <- vehicleBuyBlockedUntil;
	...
	function GetAverageCargoLoadPct(cargo) {
		local capacity = 0;
		local load = 0;
		foreach(vehicle,_ in GetVehicleList()) {
			local vehicleCapacity = AIVehicle.GetCapacity(vehicle, cargo);
			if(vehicleCapacity <= 0) continue;
			capacity += vehicleCapacity;
			load += AIVehicle.GetCargoLoad(vehicle, cargo);
		}
		if(capacity <= 0) return 0;
		return load * 100 / capacity;
	}

	function IsNetworkPaxMailWaitingCapacityMode(cargo) {
	...
	function GetRouteWaitingPressure(cargo) {
		local capacity = GetCargoCapacity(cargo);
		if(capacity <= 0 || srcHgStation == null || destHgStation == null) {
			return {
				enabled = false,
				allowed = false,
				srcWait = 0,
		...
		local srcWait = CargoUtils.GetEffectiveCargoWaiting(srcHgStation.stationId, destHgStation.stationId, cargo);
		...
		local thresholdAvg = capacity / 2;
		if(GetVehicleType() == AIVehicle.VT_AIR) {
			thresholdMax = capacity * 3 / 2;
			thresholdAvg = capacity;
		}
		local allowed = (maxWait > thresholdMax || avgWait > thresholdAvg)
				&& requiredVehicles > GetNumVehicles();
		return {
			enabled = true,
			allowed = allowed,
			srcWait = srcWait,
	...
	function GetRouteWaitingPressureLog(cargo) {
		local pressure = GetRouteWaitingPressure(cargo);
		return "waitingDemand:"+pressure.allowed
				+" srcWait:"+pressure.srcWait
				+" destWait:"+pressure.destWait
				+" avgWait:"+pressure.avgWait
				+" maxWait:"+pressure.maxWait
				+" pressure:"+pressure.pressure
				+" requiredVehicles:"+pressure.requiredVehicles
				+" capacity:"+pressure.capacity;
	}

	function GetLeftCapacityReason(cargo, isDest = false, callRoutes = null) {
		local vehicleType = GetVehicleType();
		local station = isDest ? destHgStation : srcHgStation;
		if(station == null || station.stationGroup == null) return "noStation";
		if(!isDest && vehicleType == AIVehicle.VT_WATER) return "waterSourceBypass";
		if(vehicleType == AIVehicle.VT_ROAD && IsBiDirectional()) {
			...
			return "roadBidir otherProd:"+otherProd+" thisProd:"+thisProd+" maxRouteCapacity:"+GetMaxRouteCapacity(cargo, callRoutes);
		}
		local maxCapacity;
		local prodSource = "station";
		if(IsReturnRoute(isDest)) {
			maxCapacity = srcHgStation.stationGroup.GetCurrentExpectedProduction(cargo, GetVehicleType(), true, callRoutes);
			maxCapacity = min(maxCapacity, GetMaxRouteCapacity(cargo, callRoutes));
			prodSource = "srcReturn";
		} else {
			maxCapacity = GetMaxRouteCapacity(cargo, callRoutes) / 3;
		}
		local production = station.stationGroup.GetCurrentExpectedProduction(cargo, GetVehicleType(), true, callRoutes);
		return "generic prodSource:"+prodSource+" maxCapacity:"+maxCapacity+" stationProduction:"+production+" maxRouteCapacity:"+GetMaxRouteCapacity(cargo, callRoutes);
	}

	function GetDemandDiag(cargo, isDest, cargoWaiting, needsAdditionalProducing, needsProduction) {
		if(!HogeAI.Get().IsNetworkMode() || !CargoUtils.IsPaxOrMail(cargo)) return null;
		local station = isDest ? destHgStation : srcHgStation;
		if(station == null || station.stationGroup == null) return null;
		local usingRoutes = station.stationGroup.GetUsingRoutesAsSource();
		local otherStation = isDest ? srcHgStation : destHgStation;
		local totalWaiting = AIStation.GetCargoWaiting(station.stationId, cargo);
		local viaWaiting = CargoUtils.GetEffectiveCargoWaiting(station.stationId, otherStation.stationId, cargo);
		return "RouteDemandDiag routeId:"+id
				+" trace:"+routeTraceId
				+" vt:"+GetLabel()
				+" isDest:"+isDest
				+" support:"+IsSupport()
				+" transfer:"+IsTransfer()
				+" townTransfer:"+IsTownTransferRoute()
				+" station:"+station.stationGroup.GetName()
				+" destStation:"+otherStation.stationGroup.GetName()
				+" usingRoutes:"+usingRoutes.len()
				+" totalWaiting:"+totalWaiting
				+" viaWaiting:"+viaWaiting
				+" chosenWaiting:"+cargoWaiting
				+" needsProduction:"+needsProduction
				+" needsAdditional:"+needsAdditionalProducing
				+" overflow:"+IsOverflow(cargo, isDest)
				+" leftCapacity:"+GetLeftCapacity(cargo, isDest, null)
				+" leftReason:"+GetLeftCapacityReason(cargo, isDest, null)
				+" "+GetRouteWaitingPressureLog(cargo)
				+" avgLoadPct:"+GetAverageCargoLoadPct(cargo)
				+" vehicles:"+GetVehicleList().Count()
				+" maxVehicles:"+maxVehicles
				+" "+this;
	}

	function GetCapacityBuffer(state, isReduce) {
		if(state.vehicleType == AIVehicle.VT_ROAD && state.support) {
			return isReduce ? 2 : 0;
		}
		return isReduce ? 1 : 0;
	}
	...
	function GetNetworkCapacityState(cargo, speedPct = null) {
		...
		local targetVehicles = currentVehicles;
		local pressure = null;
		if(IsNetworkPaxMailWaitingCapacityMode(cargo)) {
			pressure = GetRouteWaitingPressure(cargo);
			if(pressure.enabled) {
				targetVehicles = pressure.requiredVehicles;
			}
		} else if(capacity > 0) {
			targetVehicles = (GetProduction() + capacity - 1) / capacity;
		}
		targetVehicles = max(IsSupport() ? 1 : 0, targetVehicles);
		return {
			...
			reduceBuffer = 0,
			avgLoadPct = GetAverageCargoLoadPct(cargo),
			avgSpeedPct = speedPct != null ? speedPct : -1,
			capacity = capacity,
			pressure = pressure != null ? pressure.pressure : 0,
			requiredVehicles = pressure != null ? pressure.requiredVehicles : targetVehicles,
			srcWait = pressure != null ? pressure.srcWait : 0,
			destWait = pressure != null ? pressure.destWait : 0,
			avgWait = pressure != null ? pressure.avgWait : 0,
			maxWait = pressure != null ? pressure.maxWait : 0
		};
	}

	function GetCapacityDecisionLog(reason, action, state, strike = null, detail = null) {
		if(!HogeAI.Get().IsNetworkMode()) return null;
		local count = strike != null ? strike.count : 0;
		local firstDate = strike != null && strike.firstDate != null ? DateUtils.ToString(strike.firstDate) : "null";
		local lastDate = strike != null && strike.lastDate != null ? DateUtils.ToString(strike.lastDate) : "null";
		return "RouteCapacityDecision routeId:"+id
				+" trace:"+routeTraceId
				+" reason:"+reason
				+" action:"+action
				+" strike:"+count
				...
				+" current:"+state.currentVehicles
				+" target:"+state.targetVehicles
				+" avgTarget:"+(strike != null ? strike.avgTargetVehicles : state.targetVehicles)
				+" max:"+state.maxVehicles
				+" buyBuffer:"+state.buyBuffer
				+" reduceBuffer:"+state.reduceBuffer
				+" avgLoadPct:"+state.avgLoadPct
				+" avgSpeedPct:"+state.avgSpeedPct
				+" pressure:"+state.pressure
				+" required:"+state.requiredVehicles
				+" srcWait:"+state.srcWait
				+" destWait:"+state.destWait
	...
	function GetLeftCapacity(cargo, isDest = false, callRoutes = null) {
		...
		local maxCapacity;
		if( IsReturnRoute(isDest) ) {
			local srcProd = srcHgStation.stationGroup.GetCurrentExpectedProduction(cargo, GetVehicleType(), true, callRoutes);
			srcProd = min(srcProd, GetMaxRouteCapacity(cargo, callRoutes));
			maxCapacity = srcProd; //GetCurrentRouteCapacity(cargo);
		...
		//local engineSet = GetLatestEngineSet();
		//HgLog.Info("GetLeftCapacity:"+result+" isDest:"+isDest+" maxCapacity:"+maxCapacity+" production:"+production+"["+AICargo.GetName(cargo)+"] "+this);
		return result;
	}

	function IsVehicleBuyBlocked() {
		return HogeAI.Get().IsNetworkMode()
				&& vehicleBuyBlockedUntil != null
				&& vehicleBuyBlockedUntil > AIDate.GetCurrentDate();
	}

	function GetVehicleBuyBlockLog() {
		return "VehicleBuyBlocked reason:"+vehicleBuyBlockReason
				+" until:"+DateUtils.ToString(vehicleBuyBlockedUntil)
				+" "+this;
	}

	function BlockVehicleBuying(reason, days) {
		vehicleBuyBlockedUntil = AIDate.GetCurrentDate() + days;
		vehicleBuyBlockReason = reason;
		HgLog.Warning(GetVehicleBuyBlockLog());
	}

	function ClearReduceStrike(reason) {
		if(reduceStrikes != null && reduceStrikes.rawin(reason)) {
			reduceStrikes.rawdelete(reason);
		}
	}

	function CheckReduceStrike(reason, detail) {
		return CheckCapacityStrike(reason, detail, null).allowed;
	}

	function GetCapacityStrikeAverage(strike, key, fallback) {
		if(strike != null && strike.rawin(key) && strike.rawin("samples") && strike.samples > 0) {
			return (strike.rawget(key) + strike.samples / 2) / strike.samples;
		}
		return fallback;
	}

	function CheckCapacityStrike(reason, detail, state) {
		local result = {
			allowed = false,
			count = 0,
			firstDate = null,
		...
		if(reduceStrikes == null) {
			reduceStrikes = {};
		}
		local strike = reduceStrikes.rawin(reason) ? reduceStrikes.rawget(reason) : null;
		if(strike == null) {
			strike = {
			...
			strike.lastDate = currentDate;
			strike.count ++;
			result.advanced = true;
			if(!strike.rawin("samples")) {
				strike.samples <- max(1, strike.count - 1);
			}
			if(!strike.rawin("targetVehiclesSum")) strike.targetVehiclesSum <- (state != null ? state.targetVehicles : 0) * strike.samples;
		...
		result.count = strike.count;
		result.firstDate = strike.firstDate;
		result.lastDate = strike.lastDate;
		result.avgTargetVehicles = GetCapacityStrikeAverage(strike, "targetVehiclesSum", result.avgTargetVehicles);
		...
		if(strike.count >= 3 && currentDate - strike.firstDate >= 180) {
			reduceStrikes.rawdelete(reason);
			result.allowed = true;
		}
		return result;
	...
	function GetCargoProductions() {
		...
		if(IsBiDirectional()) {
			local cargoProds = {};
			local srcIsLarge = false;
			foreach(cargo in GetCargos()) {
				local ps = srcHgStation.stationGroup.GetExpectedProduction( cargo, GetVehicleType(), true ) + srcHgStation.GetCargoWaiting(cargo) / 8;
				local pd = destHgStation.stationGroup.GetExpectedProduction( cargo, GetVehicleType(), true ) + destHgStation.GetCargoWaiting(cargo) / 8;
				cargoProds.rawset(cargo,[ps,pd]);
				if(ps >= pd) srcIsLarge = true;
			}
			foreach(cargo,prods in cargoProds) {
				result[cargo] <- (srcIsLarge ? max(prods[0],prods[1]) : prods[0]);
				ss.push(AICargo.GetName(cargo)+"="+result[cargo]);
			}
		} else {
			foreach(cargo in GetCargos()) {
				result[cargo] <- srcHgStation.stationGroup.GetExpectedProduction( cargo, GetVehicleType(), true ) + srcHgStation.GetCargoWaiting(cargo) / 8;
				ss.push(AICargo.GetName(cargo)+"="+result[cargo]);
			}
		}
	...
	function CheckClose() {
		...
		if(srcHgStation.stationGroup == null) {
			HgLog.Warning("Route Remove (srcHgStation removed)"+GetTraceLogPart()+" "+this);
			Remove();
			return;
		}
		if(destHgStation.stationGroup == null) {
			HgLog.Warning("Route Remove (destHgStation removed)"+GetTraceLogPart()+" "+this);
			Remove();
			return;
		}
		if(srcHgStation.place != null && srcHgStation.place.IsClosed()) {
			...
			if(transfer) {
				...
				HgLog.Warning("Route Remove (src place closed)"+GetTraceLogPart()+" "+this);
				Remove();
			}
			return;
		}
		if(destHgStation.place != null && destHgStation.place.IsClosed()) {
			HgLog.Warning("Route Remove (dest place closed)"+GetTraceLogPart()+" "+this);
			Remove();
			return;
		}
		if(IsTransfer()) {
			...
			if(destRoutes.len() == 0) {
				if(destHasMultiCargos) {
					...
					HgLog.Warning("Route Remove (destStation is used by nothing)"+GetTraceLogPart()+" "+this);
					Remove();
				}
				return;
			}
			...
			if(!IsClosed() && closedAllDest) {
				if(isRemove) {
					HgLog.Warning("Route Remove (dest overflow and src share)"+GetTraceLogPart()+" "+this);
					Remove();
				} else {
			...
			if(!acceptedCargo) {
				...
				if(destHgStation.place == null) {
					...
					HgLog.Warning("Route Remove (destStation.place == null && not accept cargo)"+GetTraceLogPart()+" "+this); 
					Remove();
					return;
			...
			if(!IsSupportRaw()
				...
				foreach(route in routes) {
					...
					HgLog.Warning("Route Remove (Collided rail route found)"+GetTraceLogPart()+" "+this);
					Remove();
					return;
				}
			}
			if(!IsSupport() && GetVehicleType() == AIVehicle.VT_WATER) {
				...
				if(engineSet != null && srcHgStation.stationGroup != null) {
					if(!srcHgStation.stationGroup.IsBestEngineSetUsingSrouce(engineSet)) {
						HgLog.Warning("Route Remove (more profitable route or transfer found)"+GetTraceLogPart()+" "+this);
						Remove();
						return;
					}
				}
...
class CommonRoute extends Route {
	...
	static function CheckReduce(self,emergency = false) {
		...
		foreach(index, route in routeInstances) {
			...
			if(route.routeTraceId != null) {
				HgLog.Info("route.profit trace:"+route.routeTraceId+" profit:"+profit+" runningCost:"+runningCost+" infraCost:"+infraCost+" depreciation:"+depreciation+" "+route);
			} else {
			...
			if(vehicleType == AIVehicle.VT_ROAD && vehicleList.Count() > 2) {
				...
				if(totalCapacity > 0 && totalLoad * 4 < totalCapacity) {
					local detail = totalLoad+"/"+totalCapacity;
					if(HogeAI.Get().IsNetworkMode()) {
						...
						if(route.HasSufficientWaitingPressureForLoadCheck(state)) {
							...
							if(strike.pending) {
								continue;
							} else if(strike.allowed) {
								local target = max(route.IsSupport() ? 1 : 0, strike.avgTargetVehicles + state.reduceBuffer);
								local removed = route.ReduceVehiclesToTarget(target);
								HgLog.Warning(route.GetCapacityDecisionLog("low_avg_load", "reduce:"+removed, state, strike, detail));
								if(removed > 0) route.BlockVehicleBuying("low_avg_load", 365 * 2);
							} else {
								HgLog.Info(route.GetCapacityDecisionLog("low_avg_load", "hold", state, strike, detail));
							}
						} else {
							route.ClearReduceStrike("low_avg_load");
							HgLog.Info(route.GetCapacityDecisionLog("low_avg_load", "skip_demand", state, null, detail));
						}
					} else if(route.CheckReduceStrike("low_avg_load", detail)) {
						HgLog.Warning("ReduceVehiclesToHalf (low avg load "+detail+") "+route);
						route.ReduceVehiclesToHalf();
					}
				} else {
					route.ClearReduceStrike("low_avg_load");
	...
	function CheckReduceForRoadTransfer(vehiclesSpeed) { // transferは利益で削減しないのでスピードで落とす
			...
			if(HogeAI.Get().IsNetworkMode()) {
				...
				if(HasSufficientWaitingPressureForLoadCheck(state)) {
					...
					if(strike.pending) {
						...
						local target = max(IsSupport() ? 1 : 0, strike.avgTargetVehicles + state.reduceBuffer);
						local removed = ReduceVehiclesToTarget(target);
						HgLog.Warning(GetCapacityDecisionLog("support_low_avg_load", "reduce:"+removed, state, strike, detail));
						if(removed > 0) BlockVehicleBuying("support_low_avg_load", 365 * 2);
					} else {
						HgLog.Info(GetCapacityDecisionLog("support_low_avg_load", "hold", state, strike, detail));
					}
				} else {
					ClearReduceStrike("support_low_avg_load");
					HgLog.Info(GetCapacityDecisionLog("support_low_avg_load", "skip_demand", state, null, detail));
				}
			} else if(CheckReduceStrike("support_low_avg_load", detail)) {
				HgLog.Warning("ReduceVehiclesToHalf (support low avg load "+detail+") maxVehicles:"+maxVehicles+" "+this);
		...
		if(speedList.Count() >= 6) {
			local avgSpeed = ListUtils.Average(speedList);
			local maxSpeed = AIEngine.GetMaxSpeed(latestEngineSet.engine);
			if(avgSpeed < maxSpeed / 10) {
				local detail = "avgSpeed:"+avgSpeed+" maxSpeed:"+maxSpeed;
				if(HogeAI.Get().IsNetworkMode()) {
					local state = GetNetworkCapacityState(cargo, maxSpeed > 0 ? avgSpeed * 100 / maxSpeed : 0);
					state.reduceBuffer = GetCapacityBuffer(state, true);
					local strike = CheckCapacityStrike("support_low_speed", detail, state);
					if(strike.pending) {
						return;
					} else if(strike.allowed) {
						local target = max(IsSupport() ? 1 : 0, state.currentVehicles - max(1, strike.avgCurrentVehicles / 4));
						local removed = ReduceVehiclesToTarget(target);
						HgLog.Warning(GetCapacityDecisionLog("support_low_speed", "reduce:"+removed, state, strike, detail));
						if(removed > 0) BlockVehicleBuying("support_low_speed", 365 * 2);
					} else {
						HgLog.Info(GetCapacityDecisionLog("support_low_speed", "hold", state, strike, detail));
					}
				} else if(CheckReduceStrike("support_low_speed", detail)) {
					HgLog.Warning("ReduceVehiclesToHalf (support avgSpeed < maxSpeed/10) maxVehicles:"+maxVehicles+" "+this);
					ReduceVehiclesToHalf();
				}
			} else {
				ClearReduceStrike("support_low_speed");
			}
		}
	}
	...
	function ReduceVehiclesToTarget(targetVehicles) {
		local vehicleList = AIVehicleList_Group(this.vehicleGroup);
		vehicleList.Valuate(AIVehicle.IsStoppedInDepot);
		vehicleList.RemoveValue(1);
		targetVehicles = max(IsSupport() ? 1 : 0, targetVehicles);
		if(vehicleList.Count() <= targetVehicles) {
			return 0;
		}
		local removeCount = vehicleList.Count() - targetVehicles;
		local removed = 0;
		foreach(v,_ in vehicleList) {
			if(removed >= removeCount) {
				break;
			}
			if(AppendRemoveOrder(v)) {
				removed ++;
			}
		}
		if(removed >= 1) {
			this.maxVehicles = min(this.maxVehicles, targetVehicles);
			this.maxVehicles = max(IsSupport() ? 1 : 0, this.maxVehicles);
			saveData.maxVehicles = this.maxVehicles;
		}
		return removed;
	}
	...
	isWaitingDestRoute = null; // まだdestrouteが完成したいないrouteかどうか
	...
	function Load(t) {
		...
		isWaitingDestRoute = saveData.rawin("isWaitingDestRoute") ? t.isWaitingDestRoute : false;
	...
	function UpdateSavedData() {
		saveData = {
			...
			isWaitingDestRoute = isWaitingDestRoute
		...
		if(vehicleType == AIVehicle.VT_WATER) {
			...
			if(currentVehicles > minNum) {
				...
				if(IsNetworkPaxMailWaitingCapacityMode(cargo)) {
					...
					if(waitingPressure.enabled && currentVehicles <= waitingPressure.requiredVehicles) {
						HgLog.Info("station busy skipped "+GetRouteWaitingPressureLog(cargo)+" currentVehicles:"+currentVehicles+" "+this);
						vehicleList.Clear();
					}
				}
...
/*				
	...
	function _CheckBuildVehicle() {
		...
		if(isWaitingProduction) {
			...
			HgLog.Info("found production(isWaitingProduction=true) "+this);
			saveData.isWaitingProduction = isWaitingProduction = false;
		}
		if(isWaitingDestRoute) {
			if(GetDestRoute() == false) {
				return;
			}
			HgLog.Info("found destRoute(isWaitingDestRoute=true) "+this);
			saveData.isWaitingDestRoute = isWaitingDestRoute = false;
		}
...
/*	|| (lastCheckProductionIndex != null && lastCheckProductionIndex != HogeAI.Get().GetEstimateProductionIndex(GetProduction())))) {このチェックは重いからやらない*/
		...
		if( AIBase.RandRange(100) < 10 && CargoUtils.IsPaxOrMail(cargo)) { // 作った時には転送が無い時がある
			...
			if(!IsOverflow(cargo,false)) { //NeedsAdditionalProducingはtown内の最大productionを得ている前提なので必要なのにfalseになる事がある
				CommonRouteBuilder.CheckTownTransferCargo(this,srcHgStation,cargo);
			}
			if(isBiDirectional && !IsOverflow(cargo,true) /*NeedsAdditionalProducing(null, true)*/) {
		...
		if(IsNetworkPaxMailWaitingCapacityMode(cargo)) {
			...
			if(waitingPressure.enabled) {
				cargoWaiting = max(cargoWaiting, waitingPressure.pressure);
			}
		}
		/*
		...
		local canBuildForWaiting = cargoWaiting > needsProduction;
		if(waitingPressure != null && waitingPressure.enabled) {
			canBuildForWaiting = waitingPressure.allowed;
		}
		if(canBuildForWaiting
				&& HogeAI.Get().IsNetworkMode()
				&& CargoUtils.IsPaxOrMail(cargo)
				&& vehicleList.Count() > 0
				&& !needsAddtinalProducing
				&& !HasLeftCapacity(cargo, false, null)
				&& (waitingPressure == null || !waitingPressure.allowed)) {
			canBuildForWaiting = false;
		}
		local capacityState = null;
		local buyStrike = null;
		if(HogeAI.Get().IsNetworkMode() && canBuildForWaiting && vehicleList.Count() > 0) {
			capacityState = GetNetworkCapacityState(cargo);
			capacityState.buyBuffer = GetCapacityBuffer(capacityState, false);
			if(capacityState.targetVehicles > capacityState.currentVehicles + capacityState.buyBuffer) {
				buyStrike = CheckCapacityStrike("buy_demand", "target:"+capacityState.targetVehicles+" current:"+capacityState.currentVehicles, capacityState);
				if(buyStrike.pending) {
					return;
				}
				if(!buyStrike.allowed) {
					HgLog.Info(GetCapacityDecisionLog("buy_demand", "hold", capacityState, buyStrike, null));
					return;
				}
			} else {
				ClearReduceStrike("buy_demand");
				HgLog.Info(GetCapacityDecisionLog("buy_demand", "skip_demand", capacityState, null, null));
				canBuildForWaiting = false;
			}
		} else if(HogeAI.Get().IsNetworkMode() && !canBuildForWaiting) {
			ClearReduceStrike("buy_demand");
		}
		if(canBuildForWaiting || (vehicleList.Count()==0 && (!isTransfer || needsAddtinalProducing))) {
			local demandDiag = GetDemandDiag(cargo, false, cargoWaiting, needsAddtinalProducing, needsProduction);
			if(demandDiag != null) HgLog.Info(demandDiag);
			...
			if(!ExistsWaiting(vehicles)) {
				...
				if(latestVehicle == null) {
					...
					latestVehicle = BuildVehicleFirst();
					firstBuild ++;
					//HgLog.Info("BuildVehicle "+this);
					if(showLog) {
						HgLog.Info("BuildVehicle "+this);
					...
					if(buildNum >= 1) {
						if(capacityState != null) {
							HgLog.Info(GetCapacityDecisionLog("buy_demand", "buy:"+buildNum, capacityState, buyStrike, null));
						}
						if(demandDiag != null) HgLog.Info(demandDiag+" buildNum:"+buildNum);
						if(inDepot==null) {
							inDepot = AIList();
							inDepot.AddList(all);
							inDepot.Valuate(AIVehicle.IsStoppedInDepot);
...
/*			
	...
	function RemoveFinished() {
		HgLog.Warning("RemoveFinished:"+GetTraceLogPart()+" "+this);
		...
		while(facilities.len() >= 1) {
			...
			switch(f.name) {
				...
				case "BuildedPath":
					...
					break;
				case "roadPath":
					RoadRoute.DemolishLines([f.tiles], f.isTram);
					break;
				case "rail":
...
class RouteBuilder extends Construction {
	...
	function Build() {
		...
		if(srcPlace != null && !GetOption("notNeedToMeetDemand",false)) {
			...
			if(needsToMeetDemand && currentProduction < 50 && srcPlace.GetAccepting().GetRoutesUsingDest().len() == 0) {
				// 場所がなくなる可能性があるので存在しない以外はあとからやる
...
class CommonRouteBuilder extends RouteBuilder {
	...
	function DoBuild() {
		...
		local isWaitingDestRoute = GetOption("isWaitingDestRoute",false);
...

		{
			...
			if(path == null) {
				...
				if(vehicleType == AIVehicle.VT_ROAD) {
					AddRollback({name="roadPath", tiles=path.GetTiles(), isTram=RoadRoute.IsTramCurrent()});
				}
				local distance = AIMap.DistanceManhattan(srcHgStation.platformTile, destHgStation.platformTile)
			...
			if(route.routeTraceId != null) {
				HgLog.Info("CommonRouteBuilder.Build succeeded trace:"+route.routeTraceId+" routeId:"+route.id+" "+route);
			} else {
	...
	function SearchSharableStation(placeOrGroup, stationType, cargo, isAccepting, infrastractureType=null) {
		...
		foreach(eachPlaceOrGroup in placeOrGroups) {
			foreach(station in HgStation.SearchStation(eachPlaceOrGroup, stationType, cargo, isAccepting)) {
				...
				if(station.CanShareByMultiRoute(infrastractureType,cargo)) {
					return station;
				}
			}
		}
